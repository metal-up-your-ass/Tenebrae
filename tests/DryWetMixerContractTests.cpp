...
// TenebraeEngine::prepare() depends on and works around: the mixer's
...
// parameter was set to. See TenebraeEngine::prepare() for the fix.
           ...
           "[dsp][drywetmixer][contract]")
{
    ...
    // This is the fix TenebraeEngine::prepare() applies: prepare(), then
           ...
           "[dsp][drywetmixer][contract]")
{
    ...
    // milliseconds later. This is why TenebraeEngine::prepare() calls
